...
def email_change_verification_email(
    *,
    ...
    url = f"{frontend_base_url.rstrip('/')}?auth=verify-email-change&token={quote(token)}"
    safe_name = escape(first_name)
    return TransactionalEmail(
        recipient=recipient,
        subject="Conferma il nuovo indirizzo email MAKA",
        text_body=(
            f"Ciao {first_name},\n\n"
            "conferma questo indirizzo email per usarlo nel tuo account MAKA:\n"
            f"{url}\n\n"
            "Il link scade e puo essere usato una sola volta. Se non hai richiesto tu il "
            "cambio, ignora questo messaggio."
        ),
        html_body=(
            "<!doctype html><html><body style=\"font-family:Arial,sans-serif;color:#171717\">"
            f"<h1 style=\"font-size:24px\">Ciao {safe_name}</h1>"
            "<p>Conferma questo indirizzo email per usarlo nel tuo account MAKA.</p>"
            f"<p><a href=\"{escape(url, quote=True)}\" "
            "style=\"background:#e2231a;color:#fff;padding:12px 18px;text-decoration:none\">"
            "Conferma nuovo indirizzo</a></p>"
            "<p>Il link scade e puo essere usato una sola volta.</p>"
            "</body></html>"
        ),
    )
